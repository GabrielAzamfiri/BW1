...
function updateStars(stelle, index, className) {
  // dichiaro la funzione. stelle, index e className sono gli argomenti della funzione
...
buttonSend.addEventListener("click", function () {
  ...
  if (feedbackElement && moreInfoElement) {
    // dichiaro che se esistono già feedbackElement e moreInfoElement allora chiama la funzione setTextResultFeedback
...
function setTextResultFeedback(selectedRating, feedbackElement) {
  if (selectedRating >= 1 && selectedRating <= 5) {
    // da 1 a 5 stelle comprese
    feedbackElement.innerText = "I'm sorry 😭. Let us know how we can improve...";
  } else if (selectedRating >= 6 && selectedRating <= 7) {
    // da 6 a 7 stelle comprese
    feedbackElement.innerText = "Thank you for your review 😊.";
  } else if (selectedRating >= 8) {
    // tutto ciò che non rientra nelle precedenti due condizioni, quindi da 8 stelle in su. se facessi else e basta anche se non seleziono nessuna stella cade in questa condizione e non sarebbe molto "awesome" XD
